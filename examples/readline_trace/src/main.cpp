...
// Note: This path needs to be updated if it doesn't match. It
//       is possible that readline was linked statically if it
//       doesn't work. In this case, you can replace this path
//       with the shell (i.e.: /usr/bin/bash)
const std::string kReadlineLibraryPath{"/usr/lib/libreadline.so"};

// clang-format off
tob::ebpfpub::IFunctionTracer::ParameterList kReadlineParameterList = {
  {
    "prompt",
...
void eventParser(
  ...
  for (const auto &event : event_list) {
    ...
    const auto &prompt =
        std::get<std::string>(event.in_field_map.at("prompt").data_var);

    std::cout << "prompt: " << prompt << ", ";

    const auto &user_input =
        std::get<std::string>(event.out_field_map.at("EXIT_CODE").data_var);
...
int main(int argc, char *argv[]) {
  ...
  auto function_tracer_exp = tob::ebpfpub::IFunctionTracer::createFromUprobe(
      "readline", kReadlineLibraryPath, kReadlineParameterList,
